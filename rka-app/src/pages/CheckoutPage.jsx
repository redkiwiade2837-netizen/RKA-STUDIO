  ...
  return (
    <div className="flex-grow w-full px-page-margin py-section-gap">
      <div className="max-w-[1200px] mx-auto">
        <h1 className="font-page-title text-page-title mb-section-gap">{t('checkout.title')}</h1>
        ...
        <form onSubmit={(e) => e.preventDefault()} className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-gutter">
          ...
          <div className="md:col-span-8 flex flex-col gap-section-gap">
            ...
            <section>
              <h2 className="font-section-header text-section-header mb-6 border-b border-primary pb-2">{t('checkout.shippingInfo')}</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-gutter gap-y-6">
                <div className="col-span-1 md:col-span-2">
                  <div className="flex items-center gap-4">
                    <label className="label-brutal w-32 mb-0 flex-shrink-0">{t('checkout.email')}</label>
                ...
                <div className="col-span-1">
                  <div className="flex items-center gap-4">
                    <label className="label-brutal w-32 mb-0 flex-shrink-0">{t('checkout.firstName')}</label>
                    <input type="text" required className="input-brutal" value={shippingInfo.firstName} onChange={handleShippingChange('firstName')} />
                  </div>
                </div>
                <div className="col-span-1">
                  <div className="flex items-center gap-4">
                    <label className="label-brutal w-32 mb-0 flex-shrink-0">{t('checkout.lastName')}</label>
                    <input type="text" required className="input-brutal" value={shippingInfo.lastName} onChange={handleShippingChange('lastName')} />
                  </div>
                </div>
                <div className="col-span-1 md:col-span-2">
                  <div className="flex items-center gap-4">
                    <label className="label-brutal w-32 mb-0 flex-shrink-0">{t('checkout.streetAddress')}</label>
                ...
                <div className="col-span-1 md:col-span-2 flex flex-col gap-y-6">
                  <div className="flex items-center gap-4">
                    <label className="label-brutal w-32 flex-shrink-0 mb-0">{t('checkout.city')}</label>
                    <input type="text" required className="input-brutal" value={shippingInfo.city} onChange={handleShippingChange('city')} />
                  </div>
                  <div className="flex items-center gap-4">
                    <label className="label-brutal w-32 flex-shrink-0 mb-0">{t('checkout.state')}</label>
                    <input type="text" required className="input-brutal" value={shippingInfo.state} onChange={handleShippingChange('state')} />
                  </div>
                  <div className="flex items-center gap-4">
                    <label className="label-brutal w-32 flex-shrink-0 mb-0">{t('checkout.zip')}</label>
            ...
            <section>
              <div className="flex items-center justify-between border-b border-primary pb-2 mb-6">
                <h2 className="font-section-header text-section-header">{t('checkout.paymentMethod')}</h2>
                <div className="flex gap-2 text-text-muted">
                  <span className="material-symbols-outlined text-[20px]" style={{fontVariationSettings: "'FILL' 0"}}>lock</span>
                  <span className="font-caption text-caption">{t('checkout.secureTransaction')}</span>
                </div>
              </div>

              <div className="space-y-6 w-full text-center py-4">
                <p className="font-body text-body text-text-muted mb-4">{t('checkout.payWithPaypal')}</p>
                {shippingComplete ? (
                  <PayPalCheckout
                    cartItems={cartItems}
          ...
          <div className="md:col-span-4">
            <div className="bg-surface p-6 sticky top-8">
              <h2 className="font-section-header text-section-header mb-6 border-b border-primary pb-2">{t('checkout.orderSummary')}</h2>
                ...
                {cartItems.map(item => (
                  <div key={item.id} className="flex gap-4">
                    ...
                    <div className="flex-grow flex flex-col justify-between">
                      ...
                      <span className="font-caption text-caption text-text-muted">{t('checkout.qty')}: {item.quantity}</span>
                    </div>
                  </div>
                ))}
              </div>
