...
function withPlaceholders(products) {
  return products
    .map(({ number, title, img, imgHover, photos, drawings, type, year }) => ({
      number,
      title,
      img: img || placeholder,
      ...
      photos: photos || [],
      drawings: drawings || [],
      // From an optional "(Type, Year)" suffix on the folder name, e.g.
      // "001 Plan House (Housing, 2024)" -> type: "Housing", year: "2024".
      // Either part (or the whole suffix) can be omitted.
      type: type || null,
      year: year || null,
    }))
